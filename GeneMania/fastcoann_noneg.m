function [coAnn,BHalf] = fastcoann(Y);
...
for ii = 1:length(sizes); 
    if sizes(ii) > 0; 
        ...
        pconst = (N-nn)*(N+nn)/N2;
        nconst =  (-2*(N-nn)*nn)./N2;
        denum = abs(pconst*(nn*nn)+nconst*nn*(N-nn));
        pconst = pconst./denum;
        nconst = nconst./denum;
        ...
        vals(offset+1:offset+len) = pconst;   % replace this with correct constant 
        
        rowsPN(offsetPN+1:offsetPN+lenPN) = myrows(pn);
        valsPN(offsetPN+1:offsetPN+lenPN) = nconst;
